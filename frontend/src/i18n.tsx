...
const translations = {
    ...
    'home.sensorOk': { pt: 'Sensor de nível respondendo', en: 'Level sensor responding', ja: '水位センサー応答中' },
    ...
    'fert.doseNowPending': { pt: 'Só os não dosados', en: 'Only the pending', ja: '未投与のみ' },
    'fert.doseNowAll': { pt: 'Todos os canais', en: 'Every channel', ja: 'すべてのチャンネル' },
    ...
    'tpa.drainNominal': { pt: 'Vazão nominal dreno (L/h)', en: 'Drain rated flow (L/h)', ja: '排水ポンプ定格流量 (L/h)' },
    'tpa.refillNominal': { pt: 'Vazão nominal recalque (L/h)', en: 'Refill rated flow (L/h)', ja: '給水ポンプ定格流量 (L/h)' },
    'tpa.nominalHint': { pt: 'O valor de catálogo da bomba, medido sem altura manométrica. A bomba real só pode ser mais lenta, então uma calibração acima disso é descartada e os tempos limite passam a usar o menor dos dois. Deixe 0 para não declarar.', en: 'The pump\u2019s catalogue figure, measured at zero head. The real pump can only be slower, so a calibration above it is discarded and the timeouts fall back to the slower of the two. Leave 0 to not declare it.', ja: 'ポンプのカタログ値（揚程ゼロでの測定）。実際のポンプはこれより遅くしかならないため、これを超えるキャリブレーションは破棄され、タイムアウトは遅い方の値を使います。宣言しない場合は0のままにします。' },
    ...
    'confirm.fertDoseNow': { pt: 'Aplicar agora as doses de hoje? O dia fica marcado como feito e não repete no horário. "Todos os canais" redosa também quem já dosou hoje — use se a dose saiu vazia.', en: "Apply today's doses now? The day is stamped as done, so the schedule will not repeat them at their hour. \"Every channel\" also re-doses the ones that already ran today — for a dose that delivered nothing.", ja: '本日分を今すぐ投与しますか？当日分は実施済みとして記録され、予定時刻には繰り返されません。「すべてのチャンネル」は投与済みのものも再投与します（実際には出ていなかった場合用）。' },
    ...
    'config.blindZone': { pt: 'Zona cega do sensor (mm)', en: 'Sensor blind zone (mm)', ja: 'センサー不感帯 (mm)' },
    'config.blindZoneHint': { pt: 'Leituras mais próximas que isto são descartadas. Veja o datasheet do seu módulo — no A02YYUW costuma ser 20 mm. Alto demais e o sensor parece morto; baixo demais e ruído vira nível.', en: 'Readings closer than this are discarded. Check your module datasheet — the A02YYUW is usually 20 mm. Too high and the sensor looks dead; too low and noise becomes a level.', ja: 'これより近い測定値は破棄されます。モジュールのデータシートを確認してください（A02YYUWは通常20mm）。大きすぎるとセンサーが故障して見え、小さすぎるとノイズが水位になります。' },
